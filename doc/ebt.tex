...
\newglossaryentry{fabric}
{
  name=Fabric,
  text=fabric,
  description={ Sous ce terme généralement admis par la communauté, un fabric est un système qui répresente
  L'idée du design réseau autour d'un \textit{fabric} est bien entendu par la communauté. Sous ce modèle, un \textit{fabric} est un système composant qui représente 'grossièrement (roughly)' la commutation. En théorie, un \textit{fabric} devrait être capable de supporter un grand nombre de conceptions de pointe y compris des schémas d'adressage et des modèles de politique. \cite{fabricExtending}}
}
