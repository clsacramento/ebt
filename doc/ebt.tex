...
{
  name=Big Data,
  description={Big Data est un terme appliqué aux ensembles de données dont la taille (ou le format) est au-delà de la capacité des outils logiciels communs de les capturer, les gérer et les traiter. Une nouvelle classe de technologies et outils ont été développés pour surmonter le challenge de créer valeur commerciale de la complexe analyse de ces données. Le terme est employé pour faire référence à ce type de données ainsi que les technologies utilisées pour les stocker et les traiter.
  \cite{IMBigData}    }
}
...
{
  name=OpenFlow,
  description={  Le protocole OpenFlow vise à standardiser 1) l'interface entre entre les applications et le contrôleur  et 2) entre le contrôleur et les éléments de commutation. \cite{SurveySDNArchi} \cite{OpenFlowStanfordSwitch}  }
}
